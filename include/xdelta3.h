...
/* For convenience, zero & initialize the xd3_config structure with
   specified flags. */
static inline void xd3_init_config(xd3_config *config,
                                   uint32_t flags)
{
  memset(config, 0, sizeof(*config));
  config->flags = flags;
 ...
 * is returned again.
 */
static inline void xd3_avail_input(xd3_stream *stream,
                                   const uint8_t *idata,
                                   usize_t isize)
{
...
static inline void xd3_blksize_add(xoff_t *blkno,
                                   usize_t *blkoff,
                                   const xd3_source *source,
                                   const usize_t add)
{
